...
def send_file(conn, file_list, file_info_dict):
    while True:
        # len_filename = conn.recv(100).decode(FORMAT)
        # conn.sendall(b"ACK")
        # filename = conn.recv(len_filename).decode(FORMAT)
        # conn.sendall(filename.encode(FORMAT))
        # len_priority = conn.recv(100).decode(FORMAT)
        # conn.sendall(b"ACK")
        # priority = conn.recv(len_priority).decode(FORMAT)
        # conn.sendall(priority.encode(FORMAT))
